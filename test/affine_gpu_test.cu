...
#include "affine_base_test_gpu.h"
...
#define REGISTER_TYPED_TESTS(DESC, TYPE)                                \
  ...
  TEST_P(AffineGPUTest_##DESC, Idempotent) {                            \
    ...
    imgdistort::testing::gpu::OriginalTensor<TYPE>(N, C, &input);       \
    /* Check C++ API */                                                 \
    thrust::device_vector<TYPE> output(input.size());                   \
    imgdistort::gpu::affine_nchw(                                       \
        N, C, 16, 32, M.data().get(), 1, input.data().get(), 32,        \
        output.data().get(), 32);                                       \
    /* Check C API */                                                   \
  ...
  TEST_P(AffineGPUTest_##DESC, Generic) {                               \
    ...
    imgdistort::gpu::affine_nchw<TYPE>(                                 \
        N, C, 16, 32, M.data().get(), N, input.data().get(), 32,        \
        output.data().get(), 32);                                       \
    /* Check C API */                                                   \
    thrust::device_vector<TYPE> output2(input.size());                  \
    imgdistort_gpu_affine_nchw_##DESC(                                  \
    ...
    thrust::host_vector<TYPE> output2_cpu(output2);                     \
                                                                        \
    std::vector<TYPE> tmp;                                              \
    imgdistort::testing::cpu::ExpectedGenericTensor<TYPE>(N, C, &tmp);  \
