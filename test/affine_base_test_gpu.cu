...
const thrust::device_vector<T> &OriginalImage() {
  static const thrust::device_vector<T> img_gpu(cpu::OriginalImage<T>());
  return img_gpu;
}

template<typename T>
const thrust::device_vector<T> &Affine1Image() {
  static const thrust::device_vector<T> img_gpu(cpu::Affine1Image<T>());
  return img_gpu;
}

template<typename T>
const thrust::device_vector<T> &Affine2Image() {
  static const thrust::device_vector<T> img_gpu(cpu::Affine2Image<T>());
  return img_gpu;
}
