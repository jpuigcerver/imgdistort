...
int affine_NCHW(lua_State* L) {
  ...
  THCudaTensor* inp = static_cast<THCudaTensor *>(luaT_checkudata(L, 1, "torch.CudaTensor"));
  THCudaTensor* out = static_cast<THCudaTensor *>(luaT_checkudata(L, 2, "torch.CudaTensor"));
  THCudaTensor* M = static_cast<THCudaTensor *>(luaT_checkudata(L, 3, "torch.CudaTensor"));
  // Check input data
  if (inp->storage == 0) {
  ...
  if (inp->size[0] <= 0 || inp->size[1] <= 0 || inp->size[2] <= 0 || inp->size[3] <= 0) {
    lua_pushfstring(L, "all dimensions of the input must be greater than or equal to 1");
    lua_error(L);
  }
