...
static const Image original_image1{
  15, 10, 3,
...
static const Image original_image2{
  11, 13, 3,
...
TYPED_TEST_P(MorphologyTest, Idempotent) {
  ...
  for (int i = 0; i < 4; ++i) {
    std::vector<int> Ms_cpu{S[i], 2 * S[i] + 1};
    const int* Ms = Allocator::CloneToDevice(Ms_cpu);
    std::vector<uint8_t> m(Ms_cpu[0] * Ms_cpu[1]); m[Ms_cpu[0] * Ms_cpu[1] / 2] = 1;
    ...
    Interface::Dilate(N, C, H, W, 1, Ms, mask, src, W, dst, W);
    {
    ...
    Interface::Erode(N, C, H, W, 1, Ms, mask, src, W, dst, W);
    {
      const std::vector<T> src_host =
          Allocator::CloneToHostVec(N * C * H * W, src);
      const std::vector<T> dst_host =
